...
For any set `S`, **GP on `S`** refers to a bunch of random variables(pdf functions) whose index is the member of the set `S` such that they can have the following properties: The bunch of variables(pdf functions) are normally multivariate distributed! 
